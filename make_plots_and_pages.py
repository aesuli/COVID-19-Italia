...
<img src="https://hitcounter.pythonanywhere.com/count/tag.svg?url=https://aesuli.github.io/COVID-19-Italia/" alt="Hits" crossorigin="anonymous" referrerpolicy="no-referrer" />
'''
...
if sys.argv[-1] != 'w':
    ...
    with open('index.md', mode='wt', encoding='utf-8') as italiafile:
        ...
        vax_values = ['prima_dose', 'seconda_dose']
        daily = ita_vax_data.pivot_table(index=['data_somministrazione'],
                                         values=vax_values,
                                         aggfunc='sum')
        ...
        ds = ColumnDataSource()
        ds.data['date'] = pd.to_datetime(cumul.index)
        for value in vax_values:
            ds.data[value] = cumul[value]
        ds.data['numero_dosi'] = cumul['numero_dosi']

        p.varea_stack(stackers=vax_values, x='date', color=Category10_10[:len(vax_values)],
                      source=ds, legend_label=[label.replace('_', ' ') for label in vax_values])
        p.vline_stack(stackers=vax_values, x='date', color=Category10_10[:len(vax_values)],
                      source=ds, legend_label=[label.replace('_', ' ') for label in vax_values])
        p.vline_stack(x='date', color='red', stackers=['numero_dosi'], source=ds,
        ...
        for value in vax_values:
            ds.data[value] = daily[value]
        p2.vbar_stack(stackers=vax_values, width=datetime.timedelta(days=1) * .8, x='date',
                      color=Category10_10[:len(vax_values)], source=ds,
                      legend_label=[label.replace('_', ' ') for label in vax_values])
        ...
        vax_by_age_dose = ita_vax_data.pivot_table(index='fascia_anagrafica', values=['prima_dose', 'seconda_dose'],
                                                   aggfunc='sum')
        ...
        ds.data['prima dose'] = vax_by_age_dose['prima_dose']
        ds.data['seconda dose'] = vax_by_age_dose['seconda_dose']
        ...
        p3.vbar(x=dodge('intervallo', -0.2, range=p3.x_range), width=0.35, source=ds, top='prima dose',
                color=Category10_10[0], name='prima dose', legend_label='prima dose')
        p3.vbar(x=dodge('intervallo', 0.2, range=p3.x_range), width=0.35, source=ds, top='seconda dose',
                color=Category10_10[1], name='seconda dose', legend_label='seconda dose')
        ...
        ds_perc.data['prima dose'] = vax_by_age_dose['prima_dose'] / ita_pop_by_range * 100
        ds_perc.data['seconda dose'] = vax_by_age_dose['seconda_dose'] / ita_pop_by_range * 100

        p4.title.text = 'Percentuali dosi effettuate rispetto alla popolazione negli intervalli di età'

        p4.vbar(x=dodge('intervallo', -0.2, range=p4.x_range), width=0.35, source=ds_perc, top='prima dose',
                color=Category10_10[0], name='prima dose', legend_label='prima dose')
        p4.vbar(x=dodge('intervallo', 0.2, range=p4.x_range), width=0.35, source=ds_perc, top='seconda dose',
                color=Category10_10[1], name='seconda dose', legend_label='seconda dose')
        ...
        legend_plot.vbar(x=[0], top=[0], color=Category10_10[2], name='popolazione', legend_label='popolazione',
                         visible=False)
        legend_plot.vbar(x=[0], top=[0], color=Category10_10[0], name='prima dose', legend_label='prima dose',
                         visible=False)
        legend_plot.vbar(x=[0], top=[0], color=Category10_10[1], name='seconda dose', legend_label='seconda dose',
                         visible=False)
        ...
        for i, regione in enumerate(regioni_vax):
            ...
            vax_by_age_dose = ita_vax_data[ita_vax_data['nome_area'] == regione].pivot_table(index='fascia_anagrafica',
                                                                                             values=['prima_dose',
                                                                                                     'seconda_dose'],
                                                                                             aggfunc='sum')
            ...
            reg_plot.vbar(x=dodge('intervallo', -0.2, range=reg_plot.x_range), width=0.35, source=ds,
        ...
        print(
            '<iframe width="560" height="315" src="https://www.youtube.com/embed/qUFxxVRy9KA" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>',
            file=italiafile)
        ...
        for i, regione in enumerate(regioni_vax):
            ...
            reg_plot_perc.vbar(x=dodge('intervallo', -0.2, range=reg_plot_perc.x_range), width=0.35, source=ds_perc,
                               top='prima dose',
